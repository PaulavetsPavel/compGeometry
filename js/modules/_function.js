...
function createCordsInput() {
	...
	dynamicCoordsInput.innerHTML =
		...
		'<label>y: <input type="number" name="secondLineStartY" value=""></label>' +
		'<span>;</span>';
...
function getVectorNormal(pointStart, pointEnd) {
	return [pointStart[1] - pointEnd[1], pointEnd[0] - pointStart[0]];
}
...
function isPointOnLineSegment(point, v1, v2) {
	const [px, py] = point;
	const [x1, y1] = v1;
	const [x2, y2] = v2;
...
// Определяет, находится ли точка внутри выпуклого многоугольника
function isPointInConvexPolygon(point, polygon) {
	//Крестовое произведение для всех рёбер: Определяет направление поворота.
	// Если все повороты одинакового знака, то точка внутри многоугольника.
	let [px, py] = point;
	...
	for (let i = 0; i < polygon.length; i++) {
		...
		if (isPointOnLineSegment(point, [x1, y1], [x2, y2])) return true;
		...
		} else if (prevCrossProduct > 0 !== crossProduct > 0) {
			return false;
		}
	}
